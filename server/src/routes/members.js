...
router.post('/', requireRole('hq_admin', 'branch_admin'), wrap(async (req, res) => {
  const scope = resolveScope(req);
  const { firstName, lastName, email, phone, volunteer_skills = [], branchId } = req.body || {};
  ...
  const { rows } = await query(
    `INSERT INTO members (id,branch_id,first_name,last_name,email,phone,volunteer_skills,engagement_score)
     VALUES ($1,$2,$3,$4,$5,$6,$7,60) RETURNING *`,
    [id, targetBranch, firstName, lastName, email || null, phone || null, Array.isArray(volunteer_skills) ? volunteer_skills : []]
  );
